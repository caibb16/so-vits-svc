...
def upload_mix_append_file(files,sfiles):
    try:
        ...
        return file_paths,{"value": json.dumps(p, indent=2), "__type__": "update"}
...
def updata_mix_info(files):
    try:
        ...
        return {"value": json.dumps(p, indent=2), "__type__": "update"}
    except Exception as e:
        if debug:
            traceback.print_exc()
        raise gr.Error(e)
...
def modelAnalysis(model_path,config_path,cluster_model_path,device,enhance,diff_model_path,diff_config_path,only_diffusion,use_spk_mix,local_model_enabled,local_model_selection):
    ...
    try:
        ...
        for i in spks:
            msg += i + " "
        return {"choices": spks, "value": spks[0], "__type__": "update"}, msg
    except Exception as e:
        if debug:
            traceback.print_exc()
        raise gr.Error(e)
...
def modelUnload():
    global model
    if model is None:
        return {"choices": [], "value": "", "__type__": "update"},"没有模型需要卸载!"
    else:
        model.unload_model()
        model = None
        torch.cuda.empty_cache()
        return {"choices": [], "value": "", "__type__": "update"},"模型卸载完毕!"
...
def vc_fn(sid, input_audio, output_format, vc_transform, auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold,k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment):
    ...
    try:
        ...
        truncated_basename = Path(input_audio).stem[:-6] if len(Path(input_audio).stem) > 6 else Path(input_audio).stem
...
def local_model_refresh_fn():
    ...
    return {"choices": choices, "value": choices[0], "__type__": "update"}
